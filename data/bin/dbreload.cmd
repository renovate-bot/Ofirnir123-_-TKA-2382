...
@echo off
rem Header: rdbms/admin/dbreload.cmd frealvar_bug-24320609/1 2016/09/10 13:10:24 frealvar Exp $
rem 
rem
rem dbreload.cmd
rem
...
rem
rem    DESCRIPTION
rem      Windows command script program that allows to execute catrelod.sql in catcon.pl
...
rem derived ORACLE_HOME, last option!
for %%F in (%cd%) do set DERIVED_HOME=%%~dpF
rem storing value of ORACLE_HOME if exists.
set OH=%ORACLE_HOME%
rem storing value of ORACLE_BASE if exists.
set OB=%ORACLE_BASE%
...
set DEFAULT_OH=ORACLE_HOME not defined, using derived ORACLE_HOME from current path.
rem Destination of downgrade logs
set DWN_DEST=\cfgtoollogs\reload
set CDB=-1
set PDBLIST=-1
set NPDBS=0
rem Value of -n
...
rem else if, we try to get the OH by using orahome
...
rem 
@echo SET PAGESIZE 0 FEEDBACK OFF VERIFY OFF HEADING OFF ECHO OFF SERVEROUTPUT ON  > %DWN_DEST%\a.sql
@echo VAR CDB VARCHAR2^(10^)^;                                                    >> %DWN_DEST%\a.sql
@echo BEGIN                                                                       >> %DWN_DEST%\a.sql
@echo    BEGIN                                                                    >> %DWN_DEST%\a.sql
@echo       EXECUTE IMMEDIATE ^'SELECT CDB FROM V$DATABASE^' INTO ^:CDB^;         >> %DWN_DEST%\a.sql
@echo    EXCEPTION WHEN OTHERS THEN                                               >> %DWN_DEST%\a.sql
@echo       IF SQLCODE IN ^( -904 ^) THEN                                         >> %DWN_DEST%\a.sql
@echo          ^:CDB^:=^'NO^'^;                                                   >> %DWN_DEST%\a.sql
@echo       END IF^;                                                              >> %DWN_DEST%\a.sql
@echo    END^;                                                                    >> %DWN_DEST%\a.sql
@echo END^;                                                                       >> %DWN_DEST%\a.sql
@echo /                                                                           >> %DWN_DEST%\a.sql
@echo SELECT ^:CDB FROM DUAL^;                                                    >> %DWN_DEST%\a.sql
@echo exit                                                                        >> %DWN_DEST%\a.sql
for /f %%i IN ('sqlplus -silent / as sysdba @%DWN_DEST%\a.sql') do set CDB=%%i
del %DWN_DEST%\a.sql
...
rem 
if "!CDB!" == "NO" (
   echo Reloading noncdb
   @echo SPOOL %DWN_DEST%\catrelod.log          
   @echo SPOOL %DWN_DEST%\catrelod.log           > %DWN_DEST%\f.sql
   @echo SET PAGESIZE 0 LINESIZE 180            >> %DWN_DEST%\f.sql
   @echo @?/rdbms/admin/catrelod.sql            >> %DWN_DEST%\f.sql
   ...
   if "!PDBLIST!" == "-1" (
      rem no, run on everything
      %ORACLE_HOME%\perl\bin\perl -I%ORACLE_HOME%\perl\lib %ORACLE_HOME%\rdbms\admin\catcon.pl -d %ORACLE_HOME%\rdbms\admin -b catrelod -l %DWN_DEST% -n !NVAL! catrelod.sql
   ) else (
      rem yes, run on inclusion
      %ORACLE_HOME%\perl\bin\perl -I%ORACLE_HOME%\perl\lib %ORACLE_HOME%\rdbms\admin\catcon.pl -d %ORACLE_HOME%\rdbms\admin -b catrelod -l %DWN_DEST% -n !NVAL! -c "!PDBLIST!" catrelod.sql
   )
)
