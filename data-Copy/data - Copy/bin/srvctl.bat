@echo off
rem 
rem $Header: opsm/utl/srvctl.sbs /nt/29 2016/11/18 10:31:53 nidietri Exp $
rem
...
@set JREDIR=C:\WINDOWS.X64_193000_db_home\jdk
@set JRE="%JREDIR%\bin\java"
@set JREJAR=%JREDIR%\jre\lib\rt.jar
@set JLIB=C:\WINDOWS.X64_193000_db_home\jlib
set EONSJAR=C:\WINDOWS.X64_193000_db_home\ons\lib\ons.jar
@set GNSJAR=%JLIB%\gns.jar
set ANTLRJAR=C:\WINDOWS.X64_193000_db_home\srvm\jlib\antlr-3.3-complete.jar
@set PATH=C:\WINDOWS.X64_193000_db_home\bin;%PATH%
set CLSCEJAR=%JLIB%\clsce.jar
set CHACONFIGJAR=%JLIB%\chaconfig.jar
set JDBCJAR=C:\WINDOWS.X64_193000_db_home\jdbc\lib\ojdbc6.jar
set LDAPJAR=%JLIB%\ldapjclnt12.jar
@set JWCCREDJAR=%JLIB%\jwc-cred.jar
set JAVA_STACK_SIZE="-Xss2048k"


if (%ORATST_SRVCTL11%)==() (
   set CLASSPATH="C:\WINDOWS.X64_193000_db_home\srvm\jlib\srvctl.jar;%JLIB%\srvm.jar;%JLIB%\srvmhas.jar;%JLIB%\srvmasm.jar;%JLIB%\supercluster-common.jar;%JLIB%\supercluster.jar;%LDAPJAR%;%JLIB%\netcfg.jar;%JREJAR%;%EONSJAR%;%GNSJAR%;%ANTLRJAR%";%CLSCEJAR%;%CHACONFIGJAR%;%JDBCJAR%;%JWCCREDJAR%
) else (
...
set ORACLE_HOME_PROP=C:\WINDOWS.X64_193000_db_home

Rem - If CRSHOME, unset ORACLE_HOME 
if exist C:\WINDOWS.X64_193000_db_home\bin\crsctl.exe (
  set ORACLE_HOME=
)
